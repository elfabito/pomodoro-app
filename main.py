from tkinter import *
import math
# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 40
reps = 0
timer = None


# ---------------------------- TIMER RESET ------------------------------- #
def reset():

    global reps
    window.after_cancel(timer)
    canvas.itemconfig(text_count, text="00:00")
    text["text"] = "TIMER"
    reps = 0

# ---------------------------- TIMER MECHANISM ------------------------------- # 
def start():
    global reps
    reps += 1

    work_sec = WORK_MIN * 60
    short_break_sec = SHORT_BREAK_MIN * 60
    long_break_sec = LONG_BREAK_MIN * 60

    if reps % 8 == 0:
        text["text"] = "LONG BREAK"
        count_down(long_break_sec)
    elif reps % 2 == 0:
        text["text"] = "BREAK"
        count_down(short_break_sec)
    else:
        text["text"] = "TIMER"
        count_down(work_sec)


# ---------------------------- COUNTDOWN MECHANISM ------------------------------- #
def count_down(count):
    global timer
    count_min = math.floor(count / 60)
    count_sec = count % 60

    if count_sec < 10:
        count_sec = f"0{count_sec}"
    canvas.itemconfig(text_count, text=f"{count_min}:{count_sec}")
    if count > 0:
        timer = window.after(1000, count_down, count - 1)
    else:

        if reps % 2 != 0:
            text2["text"] += "✔"
        if reps % 8 != 0:
            start()
# ---------------------------- UI SETUP ------------------------------- #


window = Tk()
window.title("Pomodoro")
window.config(padx=100, pady=50, bg=YELLOW)


canvas = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=0)
tomato_img = PhotoImage(file="tomato.png")
canvas.create_image(100, 112, image=tomato_img)
text_count = canvas.create_text(100,130, text="00:00", fill="white", font=(FONT_NAME, 35, "bold"))
canvas.grid(row=1, column=1)


button = Button(text="Start", command=start)
button.grid(row=2, column=0)
button_reset = Button(text="Reset", command=reset)
button_reset.grid(row=2, column=2)


text = Label(text="TIMER", fg=GREEN, bg=YELLOW, font=(FONT_NAME, 35, "normal"))
text.grid(row=0, column=1)
text.config(padx=2,pady=2)

text2 = Label(text="", fg=GREEN, bg=YELLOW, font=(FONT_NAME, 15, "normal"))
text2.grid(row=4, column=1)
text2.config(padx=2,pady=2)


window.mainloop()